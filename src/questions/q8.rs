...
pub fn p2(input: String) -> u128 {
    ...
    // here, I need to check the size until it's the size of all nodes, when that comes I'll return the x cord multiplied
    let max_circuit_size = coords.len() as u128;
    ...
    while let Some(Reverse((_, p1, p2))) = min_heap.pop() {
        ...
        visited.clear();
        if circuit_size(&mut visited, &edges, p1) == max_circuit_size {
            return (p1.0 * p2.0) as u128;
        }
    }
